...
**Trigger**: Successful execution of a version bump in `PROJECT_METADATA.md` or completion of the `09-release-project` workflow.
...
2.  **Naming**: The output zip must be named `antigravity-agent-v[VERSION].zip` (e.g., `antigravity-agent-v2.1.0.zip`).
...
```powershell
# Example command for v2.1.0
Compress-Archive -Path ".agent", "README.md", "LICENSE.md", "DEPLOY.md" -DestinationPath "zip/antigravity-agent-v2.1.0.zip" -Force
```
